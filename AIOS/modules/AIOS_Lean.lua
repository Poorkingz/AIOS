...
function Lean:getMetricsReport()
    ...
    return {
        memoryReduction = string.format("%.1f%%", reduction),
        ...
        currentMode = self._performanceMode,
...
function Lean:getStatus()
    local metrics = self:getMetricsReport()
    return string.format("AIOS_Lean v%s | Mode: %s | Memory saved: %s | Strings: %d | Tables: %d",
        self._version, self._performanceMode, metrics.memoryReduction,
        metrics.stringsInterned, metrics.tablesRecycled)
end
