...
function U.ResolveService(name)
    ...
    if not service then
        error(("Service '%s' not registered."):format(name), 2)
    end
    if not service.instance then
        service.instance = service.constructor()
    end
...
function U.Await(promise)
    ...
    -- Suspend the coroutine until promise resolves
...
-- Convenience function for coroutine-based async workflows
...
function U.Delay(ms)
    return U.CreatePromise(function(resolve)
        ...
        if Timers.After then
            ...
            -- Fallback to C_Timer if available
            if _G.C_Timer and _G.C_Timer.After then
                _G.C_Timer.After(ms / 1000, resolve)
            else
                resolve() -- Immediate resolve if no timer available
            end
        end
    end)
end
...
function U.ReactiveState(initialValue, onChangeCallback)
    ...
    local mt = {
        ...
        __newindex = function(_, k, v)
            if initialValue[k] == v then return end -- No change
...
-- ========= Advanced Metaprogramming Utilities =========
