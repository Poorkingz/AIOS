...
- Donations: https://aioswow.info/donate

Purpose:
Centralized configuration system for AIOS ecosystem.
...
Config._metrics = { schemas=0, changes=0, cacheHits=0, cacheMisses=0, validationFailures=0, events=0, eventTime=0 }
...
local function runSelfTest()
    local testAppId = "AIOS_ConfigCore_SelfTest"
    local testSchema = {
        children = {
            enabled = { default = true, validate = { type = "boolean" } },
            volume = { default = 50, validate = { type = "number", min = 0, max = 100 } },
            name = { default = "Test", validate = { type = "string", pattern = "^%w+$", enum = {Test=true, Updated=true} } },
            nested = {
                children = {
                    deep = { default = 1, validate = { type = "number" } },
                    list = { default = {}, validate = { type = "table" } }
                }
            }
        }
    }

    print("AIOS Config Core: Starting self-test...")
    
    -- Clean up any previous test data
    Config._storage[testAppId] = nil
    Config._cache[testAppId] = nil
    Config._localSubs[testAppId] = nil
    Config._metrics = { schemas=0, changes=0, cacheHits=0, cacheMisses=0, validationFailures=0, events=0, eventTime=0 }
    Config._schemas[testAppId] = nil
    Config._defaults[testAppId] = nil

    -- Test 1: Schema registration
    print("Test 1: Registering schema...")
    local result = Config:RegisterSchema(testAppId, testSchema)
    if not result then 
        print("FAIL: Schema registration failed")
        return false 
    end
    print("PASS: Schema registered")

    -- Test 2: Basic Set/Get
    print("Test 2: Basic Set/Get...")
    Config:Set(testAppId, "volume", 75)
    local volume = Config:Get(testAppId, "volume")
    print("DEBUG: Set volume to 75, got:", volume)
    if volume ~= 75 then 
        print("FAIL: Expected 75, got " .. tostring(volume))
        return false 
    end
    ...
    Config:OnValidateFailed(testAppId, function() validationFailed = true end)
    Config:Set(testAppId, "volume", 150)
    if not validationFailed then 
        print("FAIL: Max validation should have failed")
        return false 
    end
    ...
    Config:BatchSet(testAppId, {
        enabled = true, 
        name = "Updated", 
        ["nested.deep"] = 2
    })
    
    local batchEnabled = Config:Get(testAppId, "enabled")
    local batchName = Config:Get(testAppId, "name") 
    local batchDeep = Config:Get(testAppId, "nested.deep")
    
    print("DEBUG: Batch results - enabled:", batchEnabled, "name:", batchName, "nested.deep:", batchDeep)
    
    if batchEnabled ~= true then 
        print("FAIL: Batch enabled not set. Got: " .. tostring(batchEnabled))
        return false 
    end
    if batchName ~= "Updated" then 
        print("FAIL: Batch name not set. Got: " .. tostring(batchName))
        return false 
    end
    if batchDeep ~= 2 then 
        print("FAIL: Batch nested.deep not set. Got: " .. tostring(batchDeep))
        return false 
    end
    print("PASS: Batch operations work")

    -- Test 5: Serialization
    print("Test 5: Serialization...")
    local serialized = Config:Serialize(testAppId)
    if not serialized then 
        print("FAIL: Serialization failed")
        return false 
    end
    print("DEBUG: Serialized length:", #serialized)
    
    -- Test 6: Deserialization
    print("Test 6: Deserialization...")
    Config._storage[testAppId] = nil
    Config._cache[testAppId] = nil
    Config:Deserialize(testAppId, serialized)
    local deserializedName = Config:Get(testAppId, "name")
    print("DEBUG: After deserialize - name:", deserializedName)
    if deserializedName ~= "Updated" then 
        print("FAIL: Deserialization failed. Expected 'Updated', got: " .. tostring(deserializedName))
        return false 
    end
    ...
    Config:Reset(testAppId)
    local resetVolume = Config:Get(testAppId, "volume")
    print("DEBUG: After reset - volume:", resetVolume)
    if resetVolume ~= 50 then 
        print("FAIL: Reset to default failed. Expected 50, got: " .. tostring(resetVolume))
        return false 
    end
    print("PASS: Reset functionality works")

    -- Test 8: Public serialization methods
    print("Test 8: Testing public TableToString and StringToTable methods...")
    local testTable = {name = "Test", value = 42, nested = {enabled = true}}
    local serializedTest = Config:TableToString(testTable)
    if not serializedTest then
        print("FAIL: TableToString failed")
        return false
    end
    
    local deserializedTest = Config:StringToTable(serializedTest)
    if not deserializedTest or deserializedTest.name ~= "Test" or deserializedTest.value ~= 42 then
        print("FAIL: StringToTable failed")
        return false
    end
    print("PASS: Public serialization methods work")

    print("AIOS Config Core: All self-tests PASSED!")
    return true
end
...
frame:SetScript("OnEvent", function(self, event, addon)
    if addon == "AIOS" and not initialized then
        initialized = true
        
        -- Register slash command
        _G.SLASH_AIOSCONFIG1 = "/aiosconfig"
        _G.SlashCmdList["AIOSCONFIG"] = function(msg)
            local result = runSelfTest()
            if result then
                print("AIOS Config Core: Self-test PASSED!")
            else
                print("AIOS Config Core: Self-test FAILED!")
            end
        end
