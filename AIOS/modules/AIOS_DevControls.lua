--[[
AIOS_DevControls.lua — Developer Console Controls
Version: 1.0
Author: Poorkingz
...
This module provides slash-command developer tooling for AIOS.
It integrates with AIOS.Debug (if available) to control logging,
buffer size, exporting, and testing — without spamming chat.

Slash API:
/aiosctl dbg on|off|toggle       → Enable/disable debug logging
/aiosctl lvl <level>             → Set log level (debug|info|warn|error)
/aiosctl size <n>                → Set ring buffer size (100–10000)
/aiosctl export                  → Export logs to CopyBox or popup frame
/aiosctl clear                   → Clear debug logs
/aiosctl test                    → Toggle QA test window (if present)

Notes:
- QUIET mode avoids chat spam, but logs everything to AIOS.Debug.
- Safe on Retail, Classic, and WoW clients back to 1.15.
- Designed to be extended later (hooks for DebugLog UI, etc).
--]]
